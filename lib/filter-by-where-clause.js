import compareCourseAgainstOperator from './compare-course-against-operator'
...
export default function filterByWhereClause(list, clause, fullList) {
    ...
    // either an and- or or-clause.
        ...
        // only 'and' and 'or' are currently supported.
            ...
            throw new ReferenceError(`neither $or nor $and could be found in ${clause}`)
        }
    }

    // where-clauses *must* be either a 'boolean' or a 'qualification'
    else {
        throw new TypeError(`wth kind of type is a "${clause.$type}" clause?`)
    }
}
    ...
    if (isObject(operator[kind])) {
        ...
        if (value.$type === 'function') {
            ...
            if (value.$name === 'max') {
                ...
                throw new ReferenceError(`${value.$name} is not a valid function to call.`)
            }
            ...
            throw new TypeError(`${value.$type} is not a valid type for a query.`)
        }
    }
