...
filter
  = 'only' _ 'courses' _ 'where' _ where:qualifier
  { return {
      $type: 'filter',
      $where: where,
  } }
  ...
  = count:(
    ...
    )+ _ ','? _ close_paren
    {
      ...
      if (count && flattened.length < count)
        throw new Error(`you requested ${count} items, but only listed ${flattened.length} options (${JSON.stringify(flattened)}).`)
...
reference 'requirement reference'
  = title:(
    ...
    {
      title = title.trim()
      if (title in options.abbreviations) {
        return {
          $type: 'reference',
          $requirement: options.abbreviations[title],
        }
      }
      else if (title in options.titles) {
        return {
          $type: 'reference',
          $requirement: options.titles[title],
        }
      }
      return {
        $type: 'reference',
