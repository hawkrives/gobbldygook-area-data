...
export function computeCourse({expr, courses, dirty}) {
    ...
    let match = assign(expr.$course, foundCourse)
    let crsid = simplifyCourse(match)
    ...
    dirty.add(crsid)
    return {computedResult: true, ...match, _used: true}
}
...
export function computeModifier({expr, ctx, courses}) {
    ...
    // count things
    if (what === 'course') {
        numCounted = countCourses(map(filtered, course => course.$course))
    }

    else if (what === 'department') {
        numCounted = countDepartments(map(filtered, course => course.$course))
    }

    else if (what === 'credit') {
        numCounted = countCredits(map(filtered, course => course.$course))
    }
...
export function computeReference({expr, ctx}) {
    ...
    let result = {computedResult: target.computed}
    ...
    if (target.hasOwnProperty('result')) {
        result.matches = target.result._matches
    }
