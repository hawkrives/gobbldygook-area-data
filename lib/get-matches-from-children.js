...
export default function getMatchesFromChildren(expr, ctx) {
    ...
    // uniq here suffers from the same problem that the dirty course stuff is
    // struggling with. (that is, uniq works on a per-object basis, and while
    // you can write down the same course for several reqs, they'll be
    // different objects.) therefore, we turn each object into a sorted JSON
    // representation of itself, and uniq based on that.
    // (I opted for passing iteratee to uniq, rather than mapping, to let lodash optimize a bit.)
    const matches = uniq(flatten(map(childKeys, key => collectMatches(ctx[key]))), stringify)
