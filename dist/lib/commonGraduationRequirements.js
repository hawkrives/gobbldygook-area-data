...
function courses(coursesTaken, creditsNeeded) {
  ...
  var noIntercollegiateSports = _.reject(coursesTaken, hasDeptNumBetween({ dept: "ESTH", start: 171, end: 194 }));
...
function ensureLimitedOffCampusCoursesDuringFinalYear(courses, fabrications) {
  ...
  var finalYearFabrications = _.chain(fabrications).filter({ year: finalYear }).reject(hasDepartment("MUSPF")) // this might do it
  .value();

  var finalYearCourses = _.filter(courses, { year: finalYear });

  return _.any([_.isEmpty(finalYearFabrications), countCredits(finalYearFabrications) <= 3]);
}
...
function residency(courses, fabrications) {
  ...
  var residencyReq = _.size(courses) - _.size(fabrications) >= 17 && ensureLimitedOffCampusCoursesDuringFinalYear(courses, fabrications);
...
function interim(courses, fabrications, graduation) {
  ...
  var interimRequirement = _.any([interimCourseCount >= 3, interimCourseCount >= 2 && finalSummerSessionCourseCount >= 1]);
...
function finalTwoYearsInResidence(courses, fabrications) {
  ...
  if (_.size(years) >= 2) {
    ...
    var hasFabricationsInFinalYears = _.every([_.isEmpty(finalYearFabrications), _.isEmpty(secondFinalYearFabrications)]);
...
exports.ensureLimitedOffCampusCoursesDuringFinalYear = ensureLimitedOffCampusCoursesDuringFinalYear;
exports.residency = residency;
exports.interim = interim;
exports.gpa = gpa;
exports.courseLevel = courseLevel;
exports.gradedCourses = gradedCourses;
exports.artsAndMusicDoubleMajor = artsAndMusicDoubleMajor;