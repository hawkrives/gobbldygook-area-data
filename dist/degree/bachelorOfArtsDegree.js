...
function beyondTheMajor(studies, courses) {
  ...
  isMajoringIn("Studio Art", studies), isMajoringIn("Art History", studies)]);

  var artMajorAndBeyond = null;
  if (isDedicatedArtist) {
    // Check the two majors agains the 18-course requirement
    artMajorAndBeyond = _(majors).filter(onlyTheTwoArtMajors).every(utilities.creditsBeyondTheArea(courses, 18)).value();

    // Remove the two majors, so they aren't checked against the 21-course
    // requirement.
    majors = _(majors).reject({ title: "Studio Art" }).reject({ title: "Art History" }).value();
  }
...
function checkBachelorOfArtsDegree(student) {
  return student.data().then(function (studentPieces) {
    ...
    var graduationRequirements = [common.courses(courses, creditsNeeded), common.residency(courses, fabrications), common.interim(courses, fabrications, graduation), common.courseLevel(courses), common.gpa(courses), common.gradedCourses(courses, fabrications), artsMajor(studies, courses), beyondTheMajor(studies, courses)];
    ...
    var educationRequirements = {
      foundation: [educ.firstYearWriting(courses, matriculation), educ.writingInContext(courses), educ.foreignLanguage(courses), educ.oralCommunication(courses), educ.abstractAndQuantitativeReasoning(courses), educ.studiesInPhysicalMovement(courses)],
      core: [educ.historicalStudiesInWesternCulture(courses), educ.studiesInHumanBehaviorAndSociety(courses), educ.multiculturalDomesticStudies(courses), educ.multiculturalGlobalStudies(courses), educ.artisticStudies(courses), educ.literaryStudies(courses), educ.biblicalStudies(courses, matriculation), educ.theologicalStudies(courses), educ.scientificExplorationAndDiscovery(courses), educ.integratedScientificTopics(courses)],
...
var bachelorOfArtsDegree = {
  ...
  _requirements: {
    artsMajor: artsMajor,
    beyondTheMajor: beyondTheMajor } };

module.exports = bachelorOfArtsDegree;